...
QUnit.test 'annotate 1 kanji with 2 hiragana', (assert) ->
	...
	assert.equal $('#1kanji_2hiragana').html(), BirdRubyXml, 'ruby xml is generated'
...
QUnit.test "annotate multiple furigana", (assert) ->
	...
	assert.equal $('#multiple_furigana').html(), 'japanese_language:<ruby><rb>日</rb><rp>(</rp><rt>に</rt><rp>)</rp></ruby><ruby><rb>本</rb><rp>(</rp><rt>ほん</rt><rp>)</rp></ruby><ruby><rb>語</rb><rp>(</rp><rt>ご</rt><rp>)</rp></ruby>', 	'ruby xml is generated'

QUnit.test "use @@ as delimiters", (assert) ->
	$('#delimiter_at').rubyann delimiters:'@@'
	assert.equal $('#delimiter_at').html(), BirdRubyXml, 'ruby xml is generated'

QUnit.test "use [] as delimiters", (assert) ->
	$('#delimiter_squarebrackets').rubyann delimiters:'[]'
	assert.equal $('#delimiter_squarebrackets').html(), BirdRubyXml, 'ruby xml is generated'
...
QUnit.test "embedded html works", (assert) ->
	...
	assert.equal $('#embedded_html').html(), "<ol><li>#{BirdRubyXml}</li></ol>", 'no html is lost'

QUnit.test "annotate with selector that processes > 1 element", (assert) ->
	$('.multi_div').rubyann()
	assert.equal $('#multidiv1').html(), "multidiv1,#{BirdRubyXml}", 'ruby xml is generated'
	assert.equal $('#multidiv2').html(), "multidiv2,#{BirdRubyXml}", 'ruby xml is generated'
